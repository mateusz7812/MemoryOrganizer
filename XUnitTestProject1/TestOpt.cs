...
namespace XUnitTestProject1
{
    public class TestOpt
    {
        ...
        public void Test1()
        {
            ...
            var strategy = new OptStrategy(physicalMemory, pageReferences);
            var frameIdEnum = correctFramesReferences.GetEnumerator();
            foreach (var pageReference in pageReferences)
            {
                if (frames.Where(f=>f.Page != null).Select(f => f.Page.Id).Contains(pageReference.PageId)) continue;
            ...
            frameIdEnum.Dispose();
            Assert.Equal(6, strategy.PagesErrors);
        }
    }
}
